...
void fill_array(char **array, int str_amount, int str_length, char is_random) {
	if (is_random) {
		for (int i = 0; i < str_amount; i++) {
			for (int j = 0; j < str_length; j++) array[i][j] = get_random_character();
			...
			while (!check) {
				for (int j = 0; j <= str_length; j++) {
					...
					if (string == '\n') {
						...
						check = 1;
						for (int k = j; k <= str_length; k++) array[i][k] = 0;
						break;
					}
					else {
						array[i][j] = string;
...
void print_array(char **array, int str_amount, int str_length, char is_backwards, char* is_answer) {
	if (!is_backwards || !*is_answer) {
		for (int i = 0; i < str_amount; i++) {
			...
			for (int j = 0; j < str_length; j++) {
				if (!*is_answer) printf( "%c" , array[i][j]);
...
int main() {
	...
	do {
		...
		printf(" Enter amount of strings you want to sort "  "(1 - 100): " );
		str_amount = get_integer();			
		
		printf(" Enter the length of strings "  "(1 - 100): " );
		str_length = get_integer();			
		
		printf(" Do you want to type strings? "  " 1 - YES, 2 - NO (generate random): ");
		is_random = make_decision();		
		
		printf("\n Choose sort order"  " 1 - (A to z), 2 - (z to A): " );
		order = make_decision();		
