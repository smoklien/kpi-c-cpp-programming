...
void print_menu() {
	...
	do {
		...
		if (decision == 'e') {
			...
			printf( "           2          3         \n");
			printf( "          a         2a    ab    \n");
			printf( "  p = b - -- ,  q = --- - -- + c"  " - Variable substitutions.\n");
			printf( "           3        27     3    \n");
			printf( "       3    2\n");
			printf( "      p    q \n");
...
void positive_discriminant(double p, double q, double d, double a) {
	...
	if (check_value(y) && check_value(u) && check_value(v)) {
		...
		if (!is_zero(y2_real))		printf("%g ", y2_real);
		if (!is_zero(y2_im))		printf("+ i * %g", y2_im);
		if (!(y2_real || y2_im))		printf("0");

		printf("\n y3 = ");

		if (!is_zero(y2_im))		printf("%g ", y2_real);
		if (!is_zero(y2_im))		printf("- i * %g", y2_im);
		if (!(y2_real || y2_im))		printf("0");
		...
		if (!is_zero(x2_real))		printf("%g ", x2_real);
		if (!is_zero(y2_im))		printf("+ i * %g", y2_im);
		if (!(x2_real || y2_im))		printf("0");
...
void solve_cubic_equation(double a, double b, double c) {
	...
	if (check_value(p) && check_value(q) && check_value(d)) {
		...
		if (d > 0)			positive_discriminant(p, q, d, a);
		else if (d == 0)	zero_discriminant(p, q, d, a);
		else				negative_discriminant(p, q, d, a);
	}
	else {
		error_message();
